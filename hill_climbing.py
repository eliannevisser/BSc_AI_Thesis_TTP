...
def hill_climber(initial_schedule, initial_home_teams, max_evaluations):
    """ 
    Implementation of the Hill Climbing (HC) algorithm used for valid initial schedule generation.
    ...
    for _ in range(max_evaluations):
        ...
        if delta <= 0:
            ...
            if sum(new_violations) <= sum(best_violations):
    ...
    return best_schedule, best_home_teams, all_mutations, all_accepted_mutations

def run_hill_climb_experiment(n_schedules, league_sizes, max_evaluations, run_nr, base_dir):
    """ 
    Function that executes an experiment using the HC algorithm.
    ...
    dest_dir = os.path.join(base_dir, f'HC_mix_run{run_nr}')
    os.makedirs(dest_dir, exist_ok=True)
    violations_file_path = os.path.join(dest_dir, f'violations_HC_mix_run{run_nr}.txt')
    total_violations_file_path = os.path.join(dest_dir, f'total_violations_HC_mix_run{run_nr}.txt')
    mutation_count_file_path = os.path.join(dest_dir, f'mutations_HC_mix_run{run_nr}.txt')
    accepted_mutation_count_file_path = os.path.join(dest_dir, f'accepted_mutation_count_HC_mix_run{run_nr}.txt')
    ...
    for n_teams in league_sizes:
        ...
        with open(violations_file_path, 'a') as f, open(mutation_count_file_path, 'a') as m, open(accepted_mutation_count_file_path, 'a') as a:
            for i in range(n_schedules):
                ...
                final_schedule, final_home_teams, all_mutations, accepted_mutations = hill_climber(initial_schedule, initial_home_teams, max_evaluations)
                ...
                schedule_file_path = os.path.join(folder_path, f'initial_schedule_HC_run{run_nr}_{i}.csv')
                with open(schedule_file_path, 'w', newline='') as schedule_file:
                    writer = csv.writer(schedule_file)
                    writer.writerows(initial_schedule)

                schedule_file_path = os.path.join(folder_path, f'final_schedule_HC_run{run_nr}_{i}.csv')
                with open(schedule_file_path, 'w', newline='') as schedule_file:
                    writer = csv.writer(schedule_file)
                    writer.writerows(final_schedule)

                home_file_path = os.path.join(folder_path, f'home_teams_per_round_HC_run{run_nr}_{i}.txt')
    ...
    print(f"Hill climbing runtime run {run_nr}: {runtime} seconds")
...
if __name__ == '__main__':
    ...
    run_nr = 1
    base_dir = '/'
    all_init_schedules, all_init_home_teams, all_final_schedules, all_final_home_teams, all_violations = run_hill_climb_experiment(n_schedules, league_sizes, max_evaluations, run_nr, base_dir)		